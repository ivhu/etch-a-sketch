    ...
    squares.forEach(square => square.addEventListener('mouseover', () => {
        if (gridClicked) {
            if (penType == 'black') {
                ...
                square.style.backgroundColor = `rgba(${Math.floor(Math.random() * 256)},
                ${Math.floor(Math.random() * 257)},${Math.floor(Math.random() * 256)})`;
            } else {
...
black.addEventListener('click', () => {penType = 'black';});

//rainbow pen button
const rainbow = document.getElementById('rainbow');
rainbow.addEventListener('click', () => {penType = 'rainbow';});
...
eraser.addEventListener('click', () => {penType = 'eraser';});
